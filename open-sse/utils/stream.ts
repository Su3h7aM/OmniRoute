			...
			start(controller) {
				...
				if (STREAM_IDLE_TIMEOUT_MS > 0) {
					idleTimer = setInterval(() => {
						if (
							!streamTimedOut &&
							Date.now() - lastChunkTime > STREAM_IDLE_TIMEOUT_MS
						) {
							streamTimedOut = true;
							...
							try {
								...
								if (isResponsesSSE) {
									...
									if (extracted) {
										...
										if (eu.prompt_tokens > 0)
											u.prompt_tokens = eu.prompt_tokens;
										if (eu.completion_tokens > 0)
											u.completion_tokens = eu.completion_tokens;
				...
				try {
					...
					if (mode === STREAM_MODE.PASSTHROUGH) {
						...
						if (shouldInjectClaudeEmptyResponseOnFlush(claudeEmptyResponseLifecycle)) {
							...
							shouldInjectClaudeMissingFinalizersOnFlush(claudeEmptyResponseLifecycle)
						) {
							emitSyntheticClaudeEmptyResponse(controller, {
							...
							try {
								...
								const responseBody = {
									choices: [
										{
											message,
											finish_reason: passthroughHasToolCalls
												? "tool_calls"
												: "stop",
										},
									],
						...
						if (parsed && !parsed.done) {
							...
							if (extracted) {
								if (!state.usage) {
									...
									if (eu.completion_tokens > 0)
										su.completion_tokens = eu.completion_tokens;
									...
									if (eu.cache_creation_input_tokens > 0)
										su.cache_creation_input_tokens =
											eu.cache_creation_input_tokens;
									if (eu.cached_tokens > 0) su.cached_tokens = eu.cached_tokens;
									if (eu.reasoning_tokens > 0)
										su.reasoning_tokens = eu.reasoning_tokens;
								}
							}
