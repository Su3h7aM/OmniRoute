...
export function normalizeCrossProxyModelId(modelId) {
	...
	const normalized =
		CROSS_PROXY_MODEL_ALIASES[modelId] ||
		CROSS_PROXY_MODEL_ALIASES_LOWER[modelId.toLowerCase()];
...
export function parseModel(modelStr) {
	...
	if (/\.\.[/\\]/.test(modelStr) || /[\x00-\x1f]/.test(modelStr)) {
		console.log(
			`[MODEL] Warning: rejected malformed model string: "${modelStr.substring(0, 50)}"`
		);
		return {
			provider: null,
			model: null,
...
export async function getModelInfoCore(modelStr, aliasesOrGetter) {
	...
	const aliases =
		typeof aliasesOrGetter === "function" ? await aliasesOrGetter() : aliasesOrGetter;
