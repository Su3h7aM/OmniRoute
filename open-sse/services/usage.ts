...
export async function getUsageForProvider(connection) {
	const { id, provider, accessToken, apiKey, providerSpecificData, projectId, email } =
		connection;
...
function inferCursorPlanName(userMeta: JsonRecord, subscriptionData: JsonRecord): string {
	...
	const planText = candidates.find(
...
async function getCursorUsage(accessToken: string) {
	try {
		if (!accessToken) {
			return {
				message:
					"Cursor token expired or unavailable. Please re-authenticate the connection.",
			};
		}
	...
	try {
		...
		if (oauthResponse.ok) {
			...
			const hasUtilization = (window: JsonRecord) =>
				...
				safePercentage(window.utilization) !== undefined;
	...
	try {
		...
		if (Object.keys(primaryWindow).length > 0) {
			const usedPercent = toNumber(
				getFieldValue(primaryWindow, "used_percent", "usedPercent"),
				0
			);
			quotas.session = {
				used: usedPercent,
