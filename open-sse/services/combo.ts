...
function getOrderedTopLevelRuntimeSteps(combo, allCombos, path = []) {
	return orderRuntimeStepsByCompositeTiers(
		getTopLevelRuntimeSteps(combo, allCombos, path),
		combo
	);
}
...
async function buildAutoCandidates(targets, comboName) {
	...
	const candidates = await Promise.all(
		targets.map(async (target) => {
			...
			const parsed = parseModel(modelStr);
			const provider =
				target.provider || parsed.provider || parsed.providerAlias || "unknown";
			const model = parsed.model || modelStr;
			...
			const circuitBreakerState =
				breakerStateRaw === "OPEN" || breakerStateRaw === "HALF_OPEN"
					? breakerStateRaw
					: "CLOSED";
...
export async function handleComboChat({
				...
				const transform = new TransformStream({
					transform(chunk, controller) {
						...
						if (
							...
							!text.includes('"finish_reason":null')
						) {
	...
	if (strategy === "weighted") {
		...
		log.info(
			"COMBO",
			`${strategy} with nested resolution: ${orderedTargets.length} total targets`
		);
	}
		...
		try {
			...
			if (lkgpProvider) {
				const lkgpIndex = orderedTargets.findIndex(
					(target) =>
						target.provider === lkgpProvider ||
						target.modelStr.startsWith(`${lkgpProvider}/`)
				);
					...
					if (connectionId) {
						...
						if (quotaInfo) {
							const resetCandidates = [
								quotaInfo.window5h?.resetAt,
								quotaInfo.window7d?.resetAt,
							]
								.filter(
	...
	for (let offset = 0; offset < modelCount; offset++) {
		...
		try {
			for (let retry = 0; retry <= maxRetries; retry++) {
				...
				if (!shouldFallback && !comboBadRequestFallback) {
					log.warn("COMBO-RR", `${modelStr} failed (no fallback)`, {
						status: result.status,
					});
					recordComboRequest(combo.name, modelStr, {
						success: false,
				...
				if ([502, 503, 504].includes(result.status) && fallbackWaitMs > 0) {
					log.info(
						"COMBO-RR",
						`Waiting ${fallbackWaitMs}ms before fallback to next model`
					);
					await new Promise((r) => setTimeout(r, fallbackWaitMs));
				}
