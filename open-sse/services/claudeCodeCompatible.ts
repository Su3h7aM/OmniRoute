...
function buildClaudeCodeCompatibleMessagesFromClaude(
	...
	const converted = Array.isArray(messages)
		? messages
				.map((message) =>
					convertClaudeCodeCompatibleClaudeMessage(message, preserveCacheControl)
				)
				.filter(
					(
						message
...
function buildClaudeCodeCompatibleToolChoice(choice: unknown) {
	...
	if (rawChoice.type === "function") {
		const functionName =
			toNonEmptyString(readRecord(rawChoice.function)?.name) ||
			toNonEmptyString(rawChoice.name);
