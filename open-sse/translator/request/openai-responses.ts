			...
			const content = Array.isArray(item.content)
				? item.content.map((contentValue) => {
						...
						if (contentItem.type === "input_file") {
							...
							if (contentItem.file_data !== undefined)
								fileObj.file_data = contentItem.file_data;
							if (contentItem.file_id !== undefined)
								fileObj.file_id = contentItem.file_id;
							if (contentItem.file_url !== undefined)
								fileObj.file_url = contentItem.file_url;
							if (contentItem.filename !== undefined)
								fileObj.filename = contentItem.filename;
						...
						? msg.content.map((contentValue) => {
								...
								if (contentItem.type === "image_url") {
									...
									const imgResult: JsonRecord = {
										type: "input_image",
										image_url:
											typeof imgUrl === "string" ? imgUrl : imgUrl?.url || "",
									};
									if (
										typeof imgUrl === "object" &&
										imgUrl?.detail !== undefined
									) {
								...
								if (contentItem.type === "file") {
									...
									if (file.file_data !== undefined)
										fileResult.file_data = file.file_data;
									if (file.file_id !== undefined)
										fileResult.file_id = file.file_id;
									if (file.file_url !== undefined)
										fileResult.file_url = file.file_url;
