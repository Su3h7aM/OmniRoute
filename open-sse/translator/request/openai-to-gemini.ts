...
function openaiToGeminiBase(model, body, _stream, toolNameOptions: GeminiToolNameOptions = {}) {
	...
	if (requestedMaxOutputTokens !== undefined) {
		result.generationConfig.maxOutputTokens = capMaxOutputTokens(
			model,
			...
			if (role === "system" && body.messages.length > 1) {
				...
				if (msg.tool_calls && Array.isArray(msg.tool_calls)) {
					...
					const firstPersistedSignature = msg.tool_calls
						.map((tc) =>
							resolveGeminiThoughtSignature(tc.id, extractClientThoughtSignature(tc))
						)
	...
	if (body.response_format) {
		if (body.response_format.type === "json_schema" && body.response_format.json_schema) {
			...
			const schema =
				body.response_format.json_schema.schema || body.response_format.json_schema;
