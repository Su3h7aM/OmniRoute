...
export class AntigravityExecutor extends BaseExecutor {
	...
	async execute({
		...
		for (let urlIndex = 0; urlIndex < fallbackCount; urlIndex++) {
			...
			let transformedBody = await this.transformRequest(
				model,
				body,
				upstreamStream,
				credentials
					...
					if (!retryMs) {
						try {
							...
							const creditsAlreadyInjected =
								(transformedBody as { enabledCreditTypes?: unknown })
									.enabledCreditTypes != null;

							if (category === "quota_exhausted" && creditsAlreadyInjected) {
								handleCreditsFailure(credentials?.accessToken || "");
								log?.warn?.(
									"AG_CREDITS",
									"Credits-first request 429'd — credits likely exhausted"
								);
							...
							if (
								...
								try {
									...
									if (
										creditsResp.ok ||
										creditsResp.status !== HTTP_STATUS.RATE_LIMITED
									) {
										log?.info?.(
											"AG_CREDITS",
											`Credits retry succeeded: ${creditsResp.status}`
										);
										if (!stream) {
											...
											try {
												const syntheticJson = await collected.response
													.clone()
													.json();
									...
									log?.warn?.(
										"AG_CREDITS",
										`Credits retry failed: ${creditsErr}`
									);
								}
							}
