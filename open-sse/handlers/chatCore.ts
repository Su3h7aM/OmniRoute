...
export async function handleChatCore({
	...
	if (
		...
		try {
			...
			if (memories.length > 0) {
				...
				log?.debug?.(
					"MEMORY",
					`Injected ${memories.length} memories for key=${apiKeyInfo.id}`
				);
			}
		} catch (memErr) {
			log?.debug?.(
						...
						msg.content = (msg.content as Record<string, unknown>[]).flatMap(
							(block: Record<string, unknown>) => {
								if (
									...
									const fileData = (block.file_url ??
										block.file ??
										block.document) as Record<string, unknown> | undefined;
									if (
										...
										const fileContent =
											...
											block.text;
										const fileName =
											(block.file as Record<string, unknown>)?.name ??
											block.name ??
											"attachment";
										if (
								...
								if (block.type === "tool_result") {
									...
									const resultContent =
										block.content ?? block.text ?? block.output ?? "";
									const resultText =
								...
								log?.debug?.(
									"CONTENT",
									`Dropped unsupported content part type="${block.type}"`
								);
								return [];
							}
						);
	...
	if (unsupported.length > 0) {
		...
		if (stripped.length > 0) {
			log?.warn?.(
				"PARAMS",
				`Stripped unsupported params for ${model}: ${stripped.join(", ")}`
			);
		}
	}
	...
	if (translatedBody && translatedBody.messages && Array.isArray(translatedBody.messages)) {
		...
		if (estimatedTokens > threshold) {
			...
			if (compressionResult.compressed) {
				...
				log?.debug?.(
					"CONTEXT",
					`Compression not applied: context already fits within target`
				);
			}
		}
	} else {
	...
	const resolveExecutorWithProxy = async (prov: string) => {
		...
		wrapper.execute = async (input: {
			...
			try {
				...
				log?.info?.(
					"UPSTREAM_PROXY",
					`${prov} native error (${errMsg}), retrying via CLIProxyAPI`
				);
				try {
					...
					log?.error?.(
						"UPSTREAM_PROXY",
						`${prov} CLIProxyAPI fallback also failed: ${proxyMsg}`
					);
					throw proxyErr;
				}
			}
	...
	if (!stream) {
		...
		const builtinToolNames = webSearchFallbackPlan.toolName
		...
		const guardrailContext = {
			...
			disabledGuardrails: resolveDisabledGuardrails({
				...
				body,
				headers:
					(clientRawRequest?.headers as Headers | Record<string, unknown> | null) ?? null,
			}),
		...
		const estimatedCost = responseUsage
			? await calculateCost(provider, model, responseUsage)
			: 0;
	...
	const onStreamComplete = ({
		...
		if (
			...
			try {
				...
				log?.debug?.(
					"CACHE",
					`Stored streaming response for ${model} (${tokensSaved} tokens)`
				);
			} catch {
