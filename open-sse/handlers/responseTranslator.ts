...
export function translateNonStreamingResponse(
	...
	if (targetFormat === FORMATS.OPENAI_RESPONSES) {
		...
		const textContent = messageSelection.text;
		...
		if (Object.keys(usage).length > 0) {
			...
			if (
				...
				const promptDetails = (result.usage as JsonRecord)
					.prompt_tokens_details as JsonRecord;
				...
				if (toNumber(usage.cache_creation_input_tokens, 0) > 0) {
					promptDetails.cache_creation_tokens = toNumber(
					...
					? candidates.map((candidateValue, index) => {
							...
							if (Array.isArray(content.parts)) {
								for (const part of content.parts) {
									...
									const inlineData = toRecord(
										partObj.inlineData ?? partObj.inline_data
									);
									if (
