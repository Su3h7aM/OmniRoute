...
function normalizeResponse(
	...
	if (providerId === "searchapi-search")
		return normalizeSearchApiResponse(data, query, searchType);
	...
	try {
		...
		if (!response.ok) {
			...
			if (log) {
				log.error(
					"SEARCH",
					`${config.id} error ${response.status}: ${errorText.slice(0, 200)}`
				);
			}
		...
		if (log) {
			log.error(
				"SEARCH",
				`${config.id} ${isTimeout ? "timeout" : "fetch error"}: ${err.message}`
			);
		}
