	...
	return toArray(rawModels).map((rawModel, index) => {
		...
		const provider =
			...
			(modelString
				...
				: target
					? "combo"
					: toString(modelRecord.provider, "unknown"));
...
async function handleGetHealth() {
	...
	try {
		...
		const resilience =
			resilienceRaw.status === "fulfilled" ? toRecord(resilienceRaw.value) : {};
		const rateLimits =
			rateLimitsRaw.status === "fulfilled" ? toRecord(rateLimitsRaw.value) : {};
		...
		const result = {
			...
			cryptography: health.cryptography
				? {
						status: toString(
							toRecord(health.cryptography).status,
							"missing_or_invalid"
						),
	...
	try {
		...
		const result = {
			combos: toArray(combos).map((rawCombo) => {
				...
				const modelsSource =
					Array.isArray(combo.models) && combo.models.length > 0
						? combo.models
						: comboData.models;
				return {
					id: comboId,
		...
		const result = {
			models: rawModels.map((rawModel) => {
				...
				return {
					...
					provider: toString(
						model.owned_by,
						toString(model.provider, args.provider || "unknown")
					),
	...
	Object.values(memoryTools).forEach((toolDef) => {
		server.registerTool(
			toolDef.name,
			{
				...
				// @ts-expect-error: dynamic zod access
				inputSchema: toolDef.inputSchema,
			},
			withScopeEnforcement(toolDef.name, async (args) => {
				try {
					const parsedArgs = toolDef.inputSchema.parse(args ?? {});
					// @ts-expect-error: handler expected specific object
