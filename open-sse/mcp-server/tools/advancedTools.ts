...
function getComboModels(combo: JsonRecord): ComboModel[] {
	...
	return sourceModels.map((model) => ({
		provider:
			getComboModelProvider(model) || (getComboModelString(model) ? "unknown" : "combo"),
...
export async function handleSimulateRoute(args: {
	...
	try {
		...
		const combos =
			combosRaw.status === "fulfilled" ? normalizeCombosResponse(combosRaw.value) : [];
		...
		const targetCombo = args.combo
			? combos.find(
					(combo) =>
						toString(combo.id) === args.combo || toString(combo.name) === args.combo
				)
		...
		if (!targetCombo) {
			return {
				content: [
					{
						type: "text" as const,
						text: JSON.stringify({ error: "No matching combo found" }),
					},
				],
				isError: true,
			};
...
export async function handleSetBudgetGuard(args: {
	...
	try {
		...
		const result = {
			...
			action: args.action,
			status:
				remaining <= 0 ? "exceeded" : remaining < args.maxCost * 0.2 ? "warning" : "active",
		};
...
export async function handleSetRoutingStrategy(args: {
	...
	try {
		...
		const combo = combos.find(
			(comboEntry) =>
				toString(comboEntry.id) === args.comboId ||
				toString(comboEntry.name) === args.comboId
		);
		...
		const result = {
			...
			combo: {
				...
				autoRoutingStrategy:
					toString(updatedCombo.strategy, args.strategy) === "auto"
...
export async function handleSetResilienceProfile(args: {
	...
	try {
		...
		if (!settings) {
			return {
				content: [
					{ type: "text" as const, text: `Error: Invalid profile "${args.profile}"` },
				],
				isError: true,
			};
		}
...
export async function handleGetProviderMetrics(args: { provider: string }) {
	...
	try {
		const [healthRaw, quotaRaw, analyticsRaw] = await Promise.allSettled([
			...
			apiFetch(
				`/api/usage/analytics?period=session&provider=${encodeURIComponent(args.provider)}`
			),
		]);
		...
		if (enabledCombos.length === 0) {
			return {
				content: [
					{
						type: "text" as const,
						text: JSON.stringify({ error: "No enabled combos available" }),
					},
				],
				isError: true,
			};
