	...
	if (!modelInfo.provider) {
		if ((modelInfo as any).errorType === "ambiguous_model") {
			...
			log.warn("CHAT", message, {
				model: modelStr,
				candidates:
					(modelInfo as any).candidateAliases ||
					(modelInfo as any).candidateProviders ||
					[],
			});
...
export async function executeChatWithBreaker({
	...
	try {
		...
		if (
			...
			/proxy unreachable/i.test(cbErr?.message || "")
		) {
...
export function handleNoCredentials(
	...
	if (credentials?.allRateLimited) {
		...
		const cooldownModel =
			typeof credentials.cooldownModel === "string" &&
			credentials.cooldownModel.trim().length > 0
