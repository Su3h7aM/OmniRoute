...
export async function getProviderCredentials(
	...
	try {
		...
		log.debug(
			...
			`${provider} | total connections: ${connections.length}, excludeIds: ${
				excludedConnectionIds.size > 0
					? Array.from(excludedConnectionIds).join(",")
					: "none"
		...
		if (connections.length === 0) {
			...
			if (allowedConnections && allowedConnections.length > 0) {
				allConnections = allConnections.filter((conn) =>
					allowedConnections.includes(conn.id)
				);
			}
			if (forcedConnectionId) {
		...
		const availableConnections = connections.filter((c) => {
			...
			if (
				requestedModel &&
				isModelExcludedByConnection(requestedModel, c.providerSpecificData)
			) {
				return false;
			}
		...
		connections.forEach((c) => {
			...
			const codexScopeLimited =
				provider === "codex" && isCodexScopeUnavailable(c, requestedModel);
			const modelLocked =
			...
			if (excluded || rateLimited) {
				...
				const scopeUntil = getCodexScopeRateLimitedUntil(
		...
		if (availableConnections.length === 0) {
			...
			const cooldownCandidates = cooldownStates
				.flatMap((state) => {
					...
					if (state.connectionCooldownMs !== null) {
						candidates.push({
							ms: state.connectionCooldownMs,
							connection: state.connection,
						});
					}
					if (state.codexScopeCooldownMs !== null) {
						candidates.push({
							ms: state.codexScopeCooldownMs,
							connection: state.connection,
						});
					}
					if (state.retryableModelCooldownMs !== null) {
						candidates.push({
							ms: state.retryableModelCooldownMs,
							connection: state.connection,
						});
					}
					return candidates;
			...
			if (earliest) {
				...
				return {
					...
					lastErrorCode: allBlockedByModelCooldown
						? 429
						: earliestConn?.errorCode || null,
		...
		if (strategy === "round-robin") {
			...
			if (candidatePool.length <= 2) {
				connection = [...candidatePool].sort((a, b) =>
					compareP2CConnections(provider, a, b)
				)[0];
			} else {
				const i =
			...
			connection =
				orderedConnections.find((c) => c.id === selectedId) || orderedConnections[0];
		} else {
...
export async function markAccountUnavailable(
	...
	try {
		...
		if (provider === "codex" && status === 429 && model && conn) {
			...
			const persistedScopeUntil = getCodexScopeRateLimitedUntil(
				conn.providerSpecificData,
				model
			);
			const scopeRateLimitedUntil = persistedScopeUntil || rateLimitedUntil;
			const scopeCooldownMs = Math.max(
				new Date(scopeRateLimitedUntil).getTime() - Date.now(),
				0
			);
