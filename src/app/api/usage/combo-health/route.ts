...
function buildProviderHealth(provider: string, snapshots: QuotaSnapshotRow[]): ProviderHealth {
	...
	for (const history of histories.values()) {
		...
		const firstSnapshot = ordered.find((entry) => {
			...
			return (
				entryView.remainingPercentage !== null &&
				entryView.remainingPercentage !== undefined
			);
		});
	...
	const firstAverage =
		firstValues.length > 0
			? firstValues.reduce((accumulator, value) => accumulator + value, 0) /
				firstValues.length
			: 0;
	const lastAverage =
		lastValues.length > 0
...
function buildConnectionHealth(
	...
	const firstSnapshot = ordered.find((entry) => {
		...
		return (
			snapshotView.remainingPercentage !== null &&
			snapshotView.remainingPercentage !== undefined
		);
	});
	const lastSnapshot = [...ordered].reverse().find((entry) => {
		const snapshotView = entry as unknown as QuotaSnapshotView;
	...
	return {
		...
		isExhausted:
			(ordered[ordered.length - 1] as unknown as QuotaSnapshotView | undefined)
				?.isExhausted === 1,
		trend,
	};
}
	...
	return new Map(
		Array.from(metrics.entries()).map(([executionKey, metric]) => [
			executionKey,
			{
				...
				successRate:
					metric.requests > 0
						? Math.round((metric.successCount / metric.requests) * 100)
						: 0,
				avgLatencyMs:
					metric.requests > 0 ? Math.round(metric.totalLatencyMs / metric.requests) : 0,
...
function buildTargetHealth(
	...
	return targets.map((target) => {
		const historicalMetric =
			historicalMetrics.get(target.executionKey) ||
			historicalMetrics.get(target.stepId) ||
			null;
		const runtimeMetric =
		...
		return {
			...
			avgLatencyMs: toSafeNumber(
				historicalMetric?.avgLatencyMs ?? runtimeMetric?.avgLatencyMs
			),
