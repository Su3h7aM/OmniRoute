...
export async function POST(request) {
	...
	try {
		...
		const { provider, modelId, modelName, source, apiFormat, supportedEndpoints } =
			validation.data;
	...
	try {
		...
		if (!model) {
			...
			if (compatOnly) {
				const knownProvider =
					!!provider &&
					(Object.hasOwn(AI_PROVIDERS as Record<string, unknown>, provider) ||
				...
				if (
					"compatByProtocol" in raw &&
					compatByProtocol &&
					typeof compatByProtocol === "object"
				) {
...
export async function PATCH(request) {
	...
	try {
		...
		for (const modelId of modelIds) {
			const updatedModel = await updateCustomModel(provider, modelId, {
