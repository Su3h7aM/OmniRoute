		...
		if (action === "authorize") {
			...
			if (provider === "qoder" && !authData.authUrl) {
				return NextResponse.json({
					...authData,
					supported: false,
					error: "Qoder browser OAuth is experimental and disabled by default. Configure QODER_OAUTH_* environment variables or use a Personal Access Token.",
				});
			}
	...
	try {
		...
		if (action === "exchange") {
			...
			if (tokenData.email) {
				...
				const match = existing.find((c: any) => {
					...
					if (!safeEqual(c.email, tokenData.email) || c.authType !== "oauth")
						return false;
					...
					if (provider === "codex" && tokenData.providerSpecificData?.workspaceId) {
						...
						return safeEqual(
							existingWorkspace,
							tokenData.providerSpecificData.workspaceId
						);
					}
					return true;
				});
			...
			if (result.success) {
				...
				if (result.tokens.email) {
					...
					const match = existing.find((c: any) => {
						...
						if (!safeEqual(c.email, result.tokens.email) || c.authType !== "oauth")
							return false;
						// For Codex, also check workspaceId to avoid overwriting different workspace connections
						if (
							provider === "codex" &&
							result.tokens.providerSpecificData?.workspaceId
						) {
							const existingWorkspace = c.providerSpecificData?.workspaceId;
							return safeEqual(
								existingWorkspace,
		...
		if (action === "poll-callback") {
			...
			try {
				...
				return NextResponse.json(
					{ success: false, error: exchangeErr.message },
					{ status: 500 }
				);
			}
		}
