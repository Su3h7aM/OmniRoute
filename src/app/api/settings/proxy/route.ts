...
function normalizeAndValidateProxy(
	...
	const type = String(proxy.type || "http").toLowerCase() as NonNullable<
...
function normalizeAndValidateProxyMap(
	...
	for (const [id, proxy] of Object.entries(proxyMap) as Array<
		[string, ProxyConfigInput | null]
	>) {
	...
	try {
		...
		if (level === "global") {
			...
			if (assignments.length > 0 && assignments[0].proxyId) {
				const proxyData = await getProxyById(assignments[0].proxyId, {
					includeSecrets: true,
				});
				if (proxyData) {
