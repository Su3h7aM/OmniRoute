		...
		<div className="p-6 max-w-7xl mx-auto space-y-6">
			...
			<Card className="p-5">
				...
				<div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
					<StatCard
						label={t("totalCalls")}
						value={status?.activity?.totalCalls24h ?? 0}
						compact
					/>
					<StatCard
						label={t("successRate")}
				...
				<div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
						...
						>
							{combos.map((combo) => (
								<option key={combo.id} value={combo.id}>
									{combo.name} (
									{combo.isActive === false ? t("inactive") : t("active")})
								</option>
							))}
						</select>
						<Button
					...
					<div className="rounded-lg border border-border p-3 space-y-3">
						...
						<select
							...
							onChange={(event) =>
								setSelectedProfile(
									event.target.value as keyof typeof RESILIENCE_PRESETS
								)
							}
						>
			...
			<Card className="p-5">
				...
				<div className="flex items-center justify-end gap-2 mt-4">
					<Button
						...
						onClick={() =>
							setAuditOffset((current) => Math.max(0, current - AUDIT_PAGE_SIZE))
						}
					>
						{t("previous")}
					</Button>
					<Button
						...
						onClick={() =>
							setAuditOffset((current) =>
								current + AUDIT_PAGE_SIZE < auditData.total
...
function StatCard({
	...
	return (
		<div
			className={`rounded-lg border border-border bg-bg p-4 ${compact ? "" : "min-h-[84px]"}`}
		>
