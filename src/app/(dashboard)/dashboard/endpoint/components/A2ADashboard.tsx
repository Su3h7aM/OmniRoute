...
export default function A2ADashboardPage() {
	...
	const handleSmokeSend = async () => {
		...
		try {
			...
			setActionMessage(
				taskId ? t("smokeSendSuccessWithTask", { taskId }) : t("smokeSendSuccess")
			);
			await refreshStatus();
			await refreshTasks();
		} finally {
			...
			if (streamTaskId) {
				setActionMessage(
					t("smokeStreamSuccessWithTask", {
						taskId: streamTaskId,
						stateSuffix: terminalState
							? `, ${t(`state.${terminalState as A2ATaskState}`)}`
							: "",
					})
				);
			} else {
		...
		<div className="p-6 max-w-7xl mx-auto space-y-6">
			<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
				...
				<StatCard
					label={t("lastTask")}
					value={
						status?.tasks?.lastTaskAt
							? new Date(status.tasks.lastTaskAt).toLocaleTimeString()
							: "—"
					}
				/>
			</div>
			...
			<div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
				<Card className="p-5">
					...
					{status?.agent ? (
						<div className="space-y-2 text-sm">
							...
							<p>
								{t("version")}:{" "}
								<span className="font-mono">{status.agent.version}</span>
							</p>
							<p>
								{t("url")}:{" "}
				...
				<Card className="p-5">
					...
					{actionMessage && (
						<p className="text-sm text-text-muted mt-3">{actionMessage}</p>
					)}
				</Card>
			</div>
			...
			<Card className="p-5">
							...
							<tbody>
								{tasksData.tasks.map((task) => {
									const fsmPhase =
										...
										(task.metadata?.workflowFSM as any)?.currentPhase;
									...
									else if (fsmPhase === "execute")
										fsmBadgeColor = "bg-blue-500/15 text-blue-500";
									else if (
										[
											"code_review",
											"quality_review",
									...
									else if (fsmPhase === "done")
										fsmBadgeColor = "bg-green-500/15 text-green-500";
									else if (fsmPhase === "failed")
										fsmBadgeColor = "bg-red-500/15 text-red-500";
				...
				<div className="flex justify-end gap-2 mt-4">
					...
					<Button
						...
						onClick={() =>
							setOffset((current) =>
								current + PAGE_SIZE < tasksData.total
									? current + PAGE_SIZE
									: current
							)
						}
