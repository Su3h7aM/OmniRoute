					...
					<div className="flex items-center gap-4">
						...
						<div>
							...
							<p className="text-xs opacity-80 mt-0.5">
								A newer version is available. Update this container deployment
								externally when convenient.
							</p>
						</div>
					</div>
					<Button
						...
						title="Update this deployment externally"
					>
						Manual Update
					</Button>
				</div>
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
					{providerStats.map((item) => (
						<ProviderOverviewCard
							...
							item={item}
							metrics={
								providerMetrics[item.provider.alias] || providerMetrics[item.id]
							}
				...
				{models.length === 0 ? (
					...
					<div className="flex flex-col gap-1 max-h-[400px] overflow-y-auto">
						{models.map((m) => (
							...
							>
								<div className="min-w-0 flex-1">
									<p className="font-mono text-sm text-text-main truncate">
										{m.fullModel}
									</p>
									{m.alias !== m.model && (
