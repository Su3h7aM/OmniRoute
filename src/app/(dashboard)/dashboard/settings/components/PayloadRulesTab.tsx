		...
		<Card>
			<div className="flex flex-col gap-5">
				<div className="flex items-start gap-3">
					...
					<div className="flex-1">
						...
						<p className="text-sm text-text-muted mt-1">
							Configure request payload mutations by model and protocol. Changes are
							persisted in settings and hot reloaded into the runtime immediately
							after save.
						</p>
					</div>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">
					<div className="rounded-lg border border-border bg-bg-secondary/40 p-3">
						...
						<p className="text-xs text-text-muted mt-1">
							Applies params only when the target path is missing from the outgoing
							payload.
						</p>
					</div>
					<div className="rounded-lg border border-border bg-bg-secondary/40 p-3">
						...
						<p className="text-xs text-text-muted mt-1">
							Forces values onto the payload, replacing anything already present at
							that path.
						</p>
					</div>
					<div className="rounded-lg border border-border bg-bg-secondary/40 p-3">
						...
						<p className="text-xs text-text-muted mt-1">
							Removes blocked params from the payload before the upstream request is
							sent.
						</p>
					</div>
					<div className="rounded-lg border border-border bg-bg-secondary/40 p-3">
						...
						<p className="text-xs text-text-muted mt-1">
							Like <code>default</code>, but string values are parsed as JSON first
							when possible. The legacy input alias <code>default-raw</code> is also
							accepted on save.
						</p>
					</div>
				</div>
					...
					<div className="flex items-center justify-between px-4 py-3 border-b border-border bg-bg-secondary/30">
						<div>
							...
							<p className="text-xs text-text-muted">
								Use the runtime schema shape: <code>default</code>,{" "}
								<code>override</code>, <code>filter</code>, <code>defaultRaw</code>.
								The API also accepts the legacy input key <code>default-raw</code>.
							</p>
						</div>
				...
				<div className="flex flex-wrap gap-2">
					...
					<Button
						onClick={handleSave}
						disabled={loading || saving || !!parsedEditor.error}
					>
