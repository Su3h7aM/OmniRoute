				...
				proxyIds.map((id) =>
					fetch(`/api/settings/proxies/assignments?proxyId=${encodeURIComponent(id)}`)
						...
						.then((data) => {
							...
							return [id, { count: assignments.length, assignments }] as [
								string,
								UsageInfo,
							];
						})
		...
		try {
			...
			if (inUse) {
				...
				const forceRes = await fetch(
					`/api/settings/proxies?id=${encodeURIComponent(id)}&force=1`,
					{
						method: "DELETE",
					}
				);
					...
					<div className="grid grid-cols-2 gap-3">
						...
						<div>
							...
							<input
								...
								value={form.username}
								placeholder={
									editingId ? "Leave blank to keep current username" : ""
								}
								onChange={(e) =>
