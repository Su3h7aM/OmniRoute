					...
					<div className="pt-1">
						<label className="relative inline-flex items-center cursor-pointer">
							<input
								...
								onChange={(e) =>
									updateSetting({ adaptiveVolumeRouting: e.target.checked })
								}
								disabled={loading}
							/>
				...
				<div className="mt-3 pt-3 border-t border-border/30 flex items-center gap-2">
					<Button
						...
						onClick={async () => {
							...
							try {
								const res = await fetch("/api/settings/lkgp-cache", {
									method: "DELETE",
								});
								...
								if (res.ok) {
									setLkgpCacheStatus({
										...
										message:
											t("lkgpCacheCleared") ||
											"LKGP cache cleared successfully",
									});
								} else {
									setLkgpCacheStatus({
										type: "error",
										message:
											data.error ||
											t("lkgpCacheClearFailed") ||
											"Failed to clear LKGP cache",
									});
								}
							} catch {
								setLkgpCacheStatus({
				...
				<div className="flex items-center gap-3 mb-4">
					...
					<div>
						...
						<p className="text-sm text-text-muted">
							Control whether OmniRoute reuses only stored Gemini thought signatures
							or accepts validated client-provided signatures in
							Antigravity-compatible tool-call flows.
						</p>
					</div>
				</div>
			...
			<Card>
				<div className="flex items-center gap-3 mb-4">
					...
					<div>
						...
						<p className="text-sm text-text-muted">
							Configure whether OmniRoute preserves client-provided cache_control
							markers
						</p>
					</div>
				</div>
