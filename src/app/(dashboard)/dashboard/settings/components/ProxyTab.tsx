...
export default function ProxyTab() {
	...
	const updateUsageTokenBuffer = async () => {
		...
		if (Number.isNaN(val) || val < 0 || val > 50000) return;
	...
	useEffect(() => {
		fetch("/api/settings", { cache: "no-store" })
			...
			.then((data) => {
				...
				const buf =
					typeof data.usageTokenBuffer === "number" ? data.usageTokenBuffer : 2000;
					...
					<div className="flex flex-col gap-3">
						<div>
							...
							<p className="text-sm text-text-muted mt-1">
								Extra tokens added to reported usage to account for system prompt
								overhead. Set to 0 to report raw provider token counts. Default:
								2000. Changes take effect within 30 seconds.
							</p>
						</div>
						<div className="flex items-center gap-3">
							<input
