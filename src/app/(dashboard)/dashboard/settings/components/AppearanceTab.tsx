		...
		try {
			...
			if (res.ok) {
				setSettings((prev) => ({
					...prev,
					[key]:
						key === HIDDEN_SIDEBAR_ITEMS_SETTING_KEY
							? normalizeHiddenSidebarItems(value)
							: value,
				}));
				...
				<div className="pt-4 border-t border-border">
					<div className="flex items-center justify-between">
						...
						<Toggle
							...
							onChange={() =>
								updateSetting("hideHealthCheckLogs", !settings.hideHealthCheckLogs)
							}
							disabled={loading}
						/>
					</div>
										...
										onChange={(e) => {
											...
											if (file) {
												if (file.size > 500 * 1024) {
													setUploadError(
														"Logo file must be less than 500KB"
													);
													return;
												}
												const validTypes = [
						...
						<div className="flex flex-col gap-2 pt-4 border-t border-border">
							...
							<div className="flex items-center gap-2">
								<input
									...
									onChange={(e) =>
										updateSetting("customFaviconUrl", e.target.value)
									}
									className="flex-1 h-10 px-3 rounded-lg bg-surface border border-border text-sm text-text-main focus:outline-none focus:border-primary"
										...
										onChange={(e) => {
											...
											if (file) {
												if (file.size > 50 * 1024) {
													setUploadError(
														"Favicon file must be less than 50KB"
													);
													return;
												}
												const validTypes = [
