...
export default function PricingTab() {
	...
	const resetProvider = useCallback(
		async (providerAlias) => {
			if (!confirm(t("resetPricingConfirm", { provider: providerAlias.toUpperCase() })))
				return;
			try {
		...
		<div className="flex flex-col gap-4">
			...
			<Card className="p-4 mt-2">
				...
				<div className="text-xs text-text-muted space-y-1">
					<p>
						{t("pricingDescInput")} • {t("pricingDescOutput")} •{" "}
						{t("pricingDescCached")} • {t("pricingDescReasoning")} •{" "}
						{t("pricingDescCacheWrite")}
					</p>
	...
	return (
		...
		>
			...
			>
				...
				<div className="flex items-center gap-3">
					{isEdited && (
						<span className="text-yellow-500 text-xs font-medium">{t("unsaved")}</span>
					)}
