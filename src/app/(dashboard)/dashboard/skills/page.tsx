...
export default function SkillsPage() {
	...
	const fetchExecutions = async (page: number) => {
		const res = await fetch(`/api/skills/executions?page=${page}&limit=20`).then((r) =>
			r.json()
		);
					...
					{!mpLoading && mpResults.length === 0 && !mpError && (
						<Card>
							<div className="text-center py-8 text-text-muted">
								Configure your SkillsMP API key in Settings to browse the
								marketplace.
							</div>
						</Card>
					)}
			...
			{activeTab === "skillssh" && (
				<div className="grid gap-4">
								...
								<Card key={skill.id}>
									<div className="flex items-center justify-between">
										<div>
											<h4 className="font-semibold">{skill.name}</h4>
											<p className="text-sm text-text-muted mt-1">
												{skill.source} · {skill.installs.toLocaleString()}{" "}
												installs
											</p>
										</div>
										<button
					...
					{!shLoading && shResults.length === 0 && !shError && (
						<Card>
							<div className="text-center py-8 text-text-muted">
								Search the skills.sh open directory to discover and install agent
								skills.
							</div>
						</Card>
					)}
