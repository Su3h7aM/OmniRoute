...
function StatCard({
	...
	return (
		...
		>
			...
			{sub && (
				<div className={`mt-1 text-text-muted ${isHero ? "text-sm" : "text-xs"}`}>
					{sub}
				</div>
			)}
		</div>
		...
		<div className="w-full self-start rounded-3xl border border-border/30 bg-surface/20 p-5">
			...
			<div className="mt-5 grid gap-3 md:grid-cols-3">
				<DetailStat
					...
					value={totalCachedRequests.toLocaleString()}
				/>
...
export default function CachePage() {
	...
	const providerEntries = pc
		? Object.entries(pc.byProvider).sort(([, left], [, right]) => {
				...
				const leftRate =
					leftTotalRequests > 0 ? (leftCachedRequests / leftTotalRequests) * 100 : 0;
				const rightRate =
												...
												<tbody>
													{providerEntries.map(([provider, data]) => {
														const cachedRequests =
															data.cachedRequests ?? data.requests;
														const totalRequests =
															data.totalRequests ?? cachedRequests;
														const cacheRate =
															totalRequests > 0
						...
						<div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
							<div className="rounded-2xl border border-border/30 bg-surface/20 p-5">
								...
								<div className="mt-4 grid gap-3 md:grid-cols-3">
									...
									<DetailStat
										label={t("total")}
										value={semanticTotalRequests.toLocaleString()}
									/>
								</div>
							</div>
