...
describe("MemoryCards", () => {
  ...
  describe("handles API errors", () => {
    it("renders error message when error prop is provided", () => {
      const { getByText } = render(<MemoryCards {...defaultProps} error="Failed to load cache stats" />);
      expect(getByText(/failed to load cache stats/i)).toBeInTheDocument();
    });

    it("shows retry button on error", () => {
      const { getByRole } = render(<MemoryCards {...defaultProps} error="Network error" onRetry={vi.fn()} />);
      expect(getByRole("button", { name: /retry/i })).toBeInTheDocument();
    });

    it("calls onRetry when retry button clicked", async () => {
      const onRetry = vi.fn();
      const { getByRole } = render(<MemoryCards {...defaultProps} error="Network error" onRetry={onRetry} />);
