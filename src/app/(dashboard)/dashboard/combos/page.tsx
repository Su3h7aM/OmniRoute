...
const STRATEGY_GUIDANCE_FALLBACK = {
	...
	"context-relay": {
		...
		example:
			"Example: Codex sessions that rotate across multiple accounts near quota exhaustion.",
	},
	random: {
...
function formatComboEntryDisplay(
	...
	const modelLabel =
		builderProvider?.models?.find((model) => model.id === parsed.modelId)?.name ||
		parsed.modelId;
...
export default function CombosPage() {
	...
	const [selectedIntelligentComboId, setSelectedIntelligentComboId] = useState<string | null>(
		null
	);
					...
					<div className="inline-flex items-center gap-2 rounded-lg border border-black/8 dark:border-white/8 bg-black/[0.02] dark:bg-white/[0.02] px-2.5 py-1.5">
						...
						<span className="text-[11px] text-text-muted">
							{emailsVisible
								? getI18nOrFallback(
										t,
										"emailVisibilityStateOn",
										"Emails visible globally"
									)
								: getI18nOrFallback(
										t,
										"emailVisibilityStateOff",
										"Emails masked globally"
									)}
						</span>
					</div>
					{!showUsageGuide && (
			...
			<div className="flex flex-wrap items-center gap-2 rounded-xl border border-black/8 dark:border-white/8 bg-black/[0.02] dark:bg-white/[0.02] p-1">
				{[
					...
					},
					{
						...
						count: combos.filter(
							(combo) => getStrategyCategory(combo?.strategy) === "intelligent"
						).length,
					},
					{
						id: "deterministic",
					...
					{filteredCombos.map((combo, index) => (
						...
						>
							<ComboCard
								...
								dragDisabled={
									savingComboOrder || activeFilter !== "all" || combos.length < 2
								}
	...
	const hasPricingForModel = useCallback(
		(modelValue) => {
			...
			return providerCandidates.some(
				(candidate) => !!pricingByProvider?.[candidate]?.[modelId]
			);
		},
				...
				<div className="flex flex-col gap-3">
							...
							<div className="rounded-lg border border-black/8 dark:border-white/8 bg-black/[0.02] dark:bg-white/[0.02] p-3 mb-2">
								<div className="flex items-start justify-between gap-3">
									...
									>
										{getI18nOrFallback(
											t,
											"builderBrowseCatalog",
											"Legacy model browser"
										)}
									</button>
								</div>
								...
								<div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-3">
									...
									<div>
										...
										>
											...
											{(selectedBuilderProvider?.models || []).map(
												(model) => (
								...
								<div className="mt-2 rounded-md border border-black/8 dark:border-white/8 bg-white/70 dark:bg-white/[0.03] px-2.5 py-2">
									...
									<div className="flex flex-wrap items-center gap-1.5 mt-2">
										<Button
											...
											size="sm"
											disabled={
												!builderCandidateStep || !!builderHasDuplicate
											}
										...
										>
											...
											{builderComboRefs.map((comboRef) => (
												<option key={comboRef.id} value={comboRef.name}>
													{comboRef.name} · {comboRef.strategy} ·{" "}
													{comboRef.stepCount} step
							...
							{hasCostOptimizedPartialPricing && (
								<div className="mt-2 rounded-md border border-amber-500/20 bg-amber-500/10 px-2 py-1.5 text-[10px] text-amber-700 dark:text-amber-300 flex items-center gap-1">
									...
									<span>
										...
										t.has("warningCostOptimizedPartialPricing")
										...
										<div className="flex items-center justify-between gap-2">
											...
											<input
												...
												onChange={(e) =>
													setConfig({
														...config,
														healthCheckEnabled: e.target.checked,
													})
												}
											...
											<div>
												...
												<input
													...
													onChange={(e) =>
														setConfig({
															...config,
															handoffThreshold: e.target.value
																? Number(e.target.value)
																: undefined,
														})
													}
									...
									<dl className="grid grid-cols-1 md:grid-cols-2 gap-2 mt-3 text-sm">
										...
										<div>
											<dt className="text-[10px] uppercase tracking-wide text-text-muted">
												{getI18nOrFallback(
													t,
													"explorationRateLabel",
													"Exploration Rate"
												)}
											</dt>
											<dd className="text-text-main mt-1">
												{Math.round(
													intelligentConfig.explorationRate * 100
												)}
												%
											</dd>
										</div>
					...
					<div className="flex gap-2 pt-1">
						...
						>
							{builderStage === "basics"
								? tc("cancel")
								: getI18nOrFallback(tc, "back", "Back")}
						</Button>
						{builderStage === "review" ? (
