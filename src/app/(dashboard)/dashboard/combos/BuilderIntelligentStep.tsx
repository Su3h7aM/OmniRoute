...
import {
	type DEFAULT_INTELLIGENT_WEIGHTS,
	FACTOR_LABELS,
	MODE_PACK_OPTIONS,
...
function toProviderOptions(activeProviders: any[] = []) {
	const uniqueProviders = new Map<
		string,
		{ id: string; label: string; connectionCount: number }
	>();
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-3">
				<Card.Section>
					...
					<input
						...
						onChange={(event) =>
							updateConfig({ explorationRate: Number(event.target.value || 0) })
						}
							...
							<div className="flex items-center justify-between gap-2">
								...
								>
									{getI18nOrFallback(
										...
										FACTOR_LABELS[
											weightKey as keyof typeof DEFAULT_INTELLIGENT_WEIGHTS
										]
									)}
								</label>
								<span className="text-[11px] text-text-muted">
