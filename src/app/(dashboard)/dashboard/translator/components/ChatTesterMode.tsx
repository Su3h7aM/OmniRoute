	...
	const handleSend = async () => {
		...
		try {
			...
			const sendRes = await fetch("/api/translator/send", {
				...
				body: JSON.stringify({
					provider,
					body: providerTargetData.result || toOpenaiData.result,
				}),
			});
			...
			if (!sendRes.ok) {
				...
				steps.push({
					...
					format: targetFmt,
					content:
						fullResponse.slice(0, 5000) +
						(fullResponse.length > 5000 ? "\n... (truncated)" : ""),
					status: "done",
				});
							...
							{chatHistory.length === 0 && (
								<div className="flex flex-col items-center justify-center h-full text-text-muted py-12">
									...
									<p className="text-xs text-center max-w-xs">
										{t("chatMessageHintPrefix")}{" "}
										<strong>{FORMAT_META[clientFormat]?.label}</strong>{" "}
								...
								>
									...
									>
										<p className="text-[10px] font-semibold text-text-muted mb-1 uppercase">
											{msg.role === "user"
												? t("youWithFormat", {
														format: FORMAT_META[clientFormat]?.label,
													})
												: t("assistant")}
										</p>
