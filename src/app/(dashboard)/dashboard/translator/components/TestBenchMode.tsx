			...
			if (!sendRes.ok) {
				...
				setResults((prev) => ({
					...
					[scenario.id]: {
						...
						error:
							errData.error ||
							t("errorMessage", { message: `HTTP ${sendRes.status}` }),
						latency,
						...
						<div className="flex-1 w-full">
							...
							<Select
								...
								options={FORMAT_OPTIONS.filter((o) =>
									["openai", "claude", "gemini", "openai-responses"].includes(
										o.value
									)
								)}
							/>
					...
					<div className="p-4">
						<div className="flex items-center justify-between mb-3">
							...
							<div className="flex items-center gap-3 text-xs text-text-muted">
								...
								<span className="flex items-center gap-1">
									<span className="size-2 rounded-full bg-red-500" /> {failCount}{" "}
									{t("failed")}
								</span>
							</div>
						</div>
									...
									<div className="flex items-center gap-2">
										...
										<div>
											...
											<p className="text-[10px] text-text-muted uppercase">
												{srcMeta.label} →{" "}
												{providerOptions.find((o) => o.value === provider)
													?.label || provider}
											</p>
										</div>
									</div>
										...
										{result.status === "pass" ? (
											<div className="flex items-center justify-between">
												...
												<span className="text-text-muted">
													{result.latency}ms • {result.chunks}{" "}
													{t("chunks")}
												</span>
											</div>
										) : (
