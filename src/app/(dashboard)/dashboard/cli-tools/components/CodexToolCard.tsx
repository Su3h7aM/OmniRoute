...
export default function CodexToolCard({
	...
	useEffect(() => {
		if (codexStatus?.config) {
			...
			const effortMatch = codexStatus.config.match(
				/^model_reasoning_effort\s*=\s*"([^"]+)"/im
			);
			...
			if (migrationsBlock) {
				...
				const chunk =
					nextSectionIdx === -1
						? migrationsBlock
						: migrationsBlock.substring(0, nextSectionIdx);
						...
						<>
							<div className="flex flex-col gap-2">
								...
								{codexStatus?.config &&
									(() => {
										const parsed =
											codexStatus.config.match(/base_url\s*=\s*"([^"]+)"/);
								...
								<div className="flex items-center gap-2">
									...
									{apiKeys.length > 0 ? (
										...
										<span className="flex-1 text-xs text-text-muted px-2 py-1.5">
											{cloudEnabled
												? t("noApiKeysCreateOne")
												: t("defaultOmnirouteKey")}
										</span>
									)}
								</div>
