					...
					<div className="flex flex-col gap-5">
						...
						<div className="flex items-start gap-3 p-3 bg-blue-500/10 border border-blue-500/30 rounded-lg">
							...
							<div className="text-sm text-blue-700 dark:text-blue-300">
								...
								<p className="mt-1 text-xs opacity-80">
									...
									block for VS Code GitHub Copilot using the Azure vendor pattern.
									Select the models you want, then copy the JSON into your config
									file.
								</p>
							</div>
						</div>
						...
						<div className="flex items-start gap-3 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
							...
							<p className="text-xs text-yellow-600 dark:text-yellow-400">
								This configuration uses the Azure vendor workaround for custom model
								lists. Tested with <strong>VS Code ≥ 1.109</strong> and{" "}
								<strong>GitHub Copilot Chat ≥ v0.37</strong>. Future extension
								updates may change this behavior.
							</p>
						</div>
							...
							<div className="mt-3 grid grid-cols-2 gap-3 pl-6">
								<div>
									...
									<input
										...
										onChange={(e) =>
											setMaxInputTokens(Number(e.target.value) || 128000)
										}
										className="w-full px-3 py-1.5 bg-bg-secondary rounded-lg text-sm border border-border focus:outline-none focus:ring-1 focus:ring-primary/50"
									/>
								</div>
								<div>
									...
									<input
										...
										onChange={(e) =>
											setMaxOutputTokens(Number(e.target.value) || 16000)
										}
										className="w-full px-3 py-1.5 bg-bg-secondary rounded-lg text-sm border border-border focus:outline-none focus:ring-1 focus:ring-primary/50"
									/>
								</div>
							...
							<div>
								<div className="flex items-center justify-between mb-2">
									<div className="flex items-center gap-2">
										...
										<span className="font-medium text-sm">
											Copy Config ({selectedModels.size} model
