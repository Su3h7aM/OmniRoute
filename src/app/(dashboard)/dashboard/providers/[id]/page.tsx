...
function isModelHidden(
	...
	if (c && Object.hasOwn(c, "isHidden")) {
		return Boolean(c.isHidden);
	}
	const o = overrideMap.get(modelId);
	if (o && Object.hasOwn(o, "isHidden")) {
...
function effectiveNormalizeForProtocol(
	...
	if (pc && Object.hasOwn(pc, "normalizeToolCallId")) {
...
function effectivePreserveForProtocol(
	...
	const o = overrideMap.get(modelId);
	const pc = getProtoSlice(c, o, protocol);
	if (pc && Object.hasOwn(pc, "preserveOpenAIDeveloperRole")) {
		return Boolean(pc.preserveOpenAIDeveloperRole);
	}
	if (c && Object.hasOwn(c, "preserveOpenAIDeveloperRole")) {
		return Boolean(c.preserveOpenAIDeveloperRole);
	}
	if (o && Object.hasOwn(o, "preserveOpenAIDeveloperRole")) {
...
function anyNoPreserveCompatBadge(
	...
	if (
		c &&
		Object.hasOwn(c, "preserveOpenAIDeveloperRole") &&
	...
	if (
		o &&
		Object.hasOwn(o, "preserveOpenAIDeveloperRole") &&
	...
	for (const p of MODEL_COMPAT_PROTOCOL_KEYS) {
		...
		if (
			pc &&
			Object.hasOwn(pc, "preserveOpenAIDeveloperRole") &&
...
function ModelCompatPopover({
	...
	const updateHeaderRow = (
		id: string,
		patch: Partial<Pick<HeaderDraftRow, "name" | "value">>
	) => {
	...
	const removeHeaderRow = (id: string) => {
		setHeaderRows((prev) => {
			...
			const normalized =
				next.length === 0 ? [{ id: genHeaderRowId(), name: "", value: "" }] : next;
						...
						<div className="min-h-0 flex-1 overflow-x-hidden overflow-y-auto bg-white p-3 [scrollbar-gutter:stable] [scrollbar-width:thin] dark:bg-zinc-950">
							...
							<div className="flex flex-col gap-3.5">
								<Toggle
									...
									onChange={(v) =>
										onCompatPatch(protocol, { normalizeToolCallId: v })
									}
									disabled={disabled}
								/>
								{devToggle && (
									<Toggle
										...
										onChange={(checked) =>
											onCompatPatch(protocol, {
												preserveOpenAIDeveloperRole: !checked,
											})
										}
										disabled={disabled}
									/>
...
export default function ProviderDetailPage() {
				...
				if (providerId === "gemini" && newConnection?.id) {
					...
					try {
						const syncRes = await fetch(
							`/api/providers/${newConnection.id}/sync-models`,
							{
								method: "POST",
						...
						if (!syncRes.ok || syncData.error) {
							setImportProgress((prev) => ({
								...
								error:
									...
									syncData.error ||
									t("failedImportModels"),
							}));
							return null;
						}

						const syncedCount = syncData.syncedModels || 0;
						const syncedModelList: Array<{ id: string; name?: string }> =
							syncData.models || [];
						const logs: string[] = [];
		...
		try {
			...
			setImportProgress((prev) => ({
				...
				logs: [
					...
					...(newModels.length < fetchedModels.length
						? [
								t("skippingExistingModels", {
									count: fetchedModels.length - newModels.length,
								}) ||
			...
			for (let i = 0; i < newModels.length; i++) {
				...
				setImportProgress((prev) => ({
					...prev,
					current: i + 1,
					status: t("importingModelsProgress", {
						current: i + 1,
						total: newModels.length,
					}),
	...
	const customMap = useMemo(
		() => buildCompatMap(modelMeta.customModels),
	...
	const saveModelCompatFlags = async (modelId: string, patch: ModelCompatSavePatch) => {
		...
		try {
			...
			if (c) {
				if (onlyCompatByProtocol) {
					...
					body = {
						...
						supportedEndpoints:
							Array.isArray(c.supportedEndpoints) &&
							(c.supportedEndpoints as unknown[]).length
			...
			if (!res.ok) {
				...
				notify.error(
					detail
						? `${t("failedSaveCustomModel")} — ${detail}`
						: t("failedSaveCustomModel")
				);
				return;
			}
		} catch {
	...
	const handleBulkToggleModelHidden = async (
		...
		try {
			const res = await fetch(
				`/api/provider-models?provider=${encodeURIComponent(providerKey)}`,
				{
					method: "PATCH",
					headers: { "Content-Type": "application/json" },
			...
			<div>
				...
				{modelsWithVisibility.length > 0 && (
					<ModelVisibilityToolbar
						...
						selectAllDisabled={
							hiddenFilteredCount === 0 || bulkVisibilityAction !== null
						}
						deselectAllDisabled={
							visibleFilteredCount === 0 || bulkVisibilityAction !== null
						}
					/>
				)}
						...
						return (
							<ModelRow
								...
								getUpstreamHeadersRecord={(p) =>
									getUpstreamHeadersRecordForModel(model.id, p)
								}
								saveModelCompatFlags={saveModelCompatFlags}
					...
					<div className="flex items-center justify-between mb-4">
						<div>
							...
							<p className="text-sm text-text-muted">
								{getApiLabel()} · {(providerNode.baseUrl || "").replace(/\/$/, "")}/
								{getApiPath()}
							</p>
						</div>
						<div className="flex items-center gap-2">
								...
								title={
									proxyConfig?.providers?.[providerId]
										? t("providerProxyTitleConfigured", {
												host:
													proxyConfig.providers[providerId].host ||
													t("configured"),
											})
					...
					{connections.length === 0 ? (
						...
						(() => {
									...
									<div className="flex flex-col divide-y divide-black/[0.03] dark:divide-white/[0.03]">
										{sorted.map((conn, index) => (
											<ConnectionRow
												...
												onMoveUp={() =>
													handleSwapPriority(conn, sorted[index - 1])
												}
												onMoveDown={() =>
													handleSwapPriority(conn, sorted[index + 1])
												}
												...
												proxyHost={
													connProxyMap[conn.id]?.proxy?.host || null
												}
											/>
										))}
									</div>
							...
							for (const conn of sorted) {
								const tag =
									(
										conn.providerSpecificData?.tag as string | undefined
									)?.trim() || "";
													...
													{groupConns.map((conn, index) => (
														<ConnectionRow
															...
															isLast={
																...
																index === groupConns.length - 1
															}
															onMoveUp={() =>
																handleSwapPriority(
																	conn,
																	sorted[sorted.indexOf(conn) - 1]
																)
															}
															onMoveDown={() =>
																handleSwapPriority(
															...
															onRetest={() =>
																handleRetestConnection(conn.id)
															}
															isRetesting={retestingId === conn.id}
															onEdit={() => {
																setSelectedConnection(conn);
															...
															hasProxy={
																!!connProxyMap[conn.id]?.proxy
															}
															proxySource={
																connProxyMap[conn.id]?.level || null
															}
															proxyHost={
																connProxyMap[conn.id]?.proxy
																	?.host || null
					...
					{providerId === "perplexity-search" && (
						<div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-500/10 border border-blue-500/20">
							...
							<p className="text-xs text-blue-300">
								Uses the same API key as <strong>Perplexity</strong> (chat
								provider). If you already have Perplexity configured, no additional
								setup is needed.
							</p>
						</div>
					)}
					{providerId === "google-pse-search" && (
						<div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-500/10 border border-amber-500/20">
							...
							<p className="text-xs text-amber-200">
								Google Programmable Search requires two values: your API key and the
								Search Engine ID (<strong>cx</strong>) from the Programmable Search
								Engine dashboard.
							</p>
						</div>
					)}
					{providerId === "searxng-search" && (
						<div className="mt-3 flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
							...
							<p className="text-xs text-emerald-200">
								SearXNG is self-hosted. Configure the instance base URL here. API
								key is optional and can be left blank for public or unauthenticated
								instances. Local/private URL validation requires{" "}
									...
									{batchTestResults.summary && (
										<div className="flex items-center gap-3 text-xs mb-1">
											...
											<span className="px-2 py-0.5 rounded bg-emerald-500/15 text-emerald-400 font-medium">
												{t("passedCount", {
													count: batchTestResults.summary.passed,
												})}
											</span>
											{batchTestResults.summary.failed > 0 && (
												<span className="px-2 py-0.5 rounded bg-red-500/15 text-red-400 font-medium">
													{t("failedCount", {
														count: batchTestResults.summary.failed,
													})}
												</span>
											)}
											<span className="text-text-muted ml-auto">
												{t("testedCount", {
													count: batchTestResults.summary.total,
												})}
											</span>
										</div>
									)}
									{(batchTestResults.results || []).map((r: any, i: number) => (
										...
										>
											...
											>
												{r.valid
													? t("okShort")
													: r.diagnosis?.type || t("errorShort")}
											</span>
										</div>
									))}
									{(!batchTestResults.results ||
										batchTestResults.results.length === 0) && (
		...
		>
			...
			<div className="flex shrink-0 items-center gap-1 self-start">
				...
				<ModelCompatPopover
					...
					effectiveModelPreserveDeveloper={(p) =>
						effectiveModelPreserveDeveloper(modelId, p)
					}
					getUpstreamHeadersRecord={getUpstreamHeadersRecord}
					onCompatPatch={(protocol, payload) =>
											...
											<div className="flex min-w-0 flex-wrap items-end gap-x-3 gap-y-2">
												<div className="w-[11rem] shrink-0 min-w-0">
													...
													>
														...
														<option value="audio-speech">
															{t("audioSpeech")}
														</option>
														<option value="images-generations">
															{t("imagesGenerations")}
														</option>
													</select>
												</div>
													...
													<div className="flex flex-wrap items-center gap-x-2 sm:gap-x-3 gap-y-1 min-w-0">
														{[
															...
															>
																<input
																	type="checkbox"
																	checked={editingEndpoints.includes(
																		ep
																	)}
																	onChange={(e) => {
																		if (e.target.checked) {
		...
		try {
			...
			if (!customModelRes.ok) {
				...
				try {
					...
					console.error(
						"Failed to parse error response from custom model API:",
						jsonError
					);
				}
...
function ConnectionRow({
	...
	useEffect(() => {
		...
		updateCooldown();
		const interval = connection.rateLimitedUntil ? setInterval(updateCooldown, 1000) : null;
		return () => {
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (formData.excludedModels.trim()) {
				providerSpecificData.excludedModels = parseExcludedModelsInput(
					formData.excludedModels
				);
			}
...
function EditConnectionModal({ isOpen, connection, onSave, onClose }: EditConnectionModalProps) {
	...
	useEffect(() => {
		if (connection) {
			...
			const existingConsoleApiKey =
				typeof rawConsoleApiKey === "string" ? rawConsoleApiKey : "";
			setFormData({
				...
				apiRegion:
					(connection.providerSpecificData?.apiRegion as string) || "international",
				validationModelId:
					(connection.providerSpecificData?.validationModelId as string) || "",
				...
				codexOpenaiStoreEnabled:
					connection.providerSpecificData?.openaiStoreEnabled === true,
		...
		try {
			...
			if (!isOAuth) {
				...
				if (isCcCompatible) {
					...
					updates.providerSpecificData.requestDefaults =
						Object.keys(currentRequestDefaults).length > 0
				...
				{isCodex && (
					<div className="flex flex-col gap-4 rounded-lg border border-border/50 bg-surface/20 p-4">
						...
						<Toggle
							...
							onChange={(checked) =>
								setFormData({ ...formData, codexFastServiceTier: checked })
							}
					...
					<div className="bg-sidebar/50 p-3 rounded-lg">
						...
						<div className="flex items-center gap-2">
							<p
								...
								title={showEmail ? connection.email : undefined}
							>
					...
					<div className="flex flex-col gap-2">
						...
						{extraApiKeys.length > 0 && (
							<p className="text-[11px] text-text-muted">
								{extraApiKeys.length + 1} keys total — rotating round-robin on each
								request.
							</p>
						)}
					</div>
	...
	const handleSubmit = async () => {
		...
		try {
			const payload: any = {
				...
				chatPath:
					formData.chatPath || (isCcCompatible ? CC_COMPATIBLE_DEFAULT_CHAT_PATH : ""),
		...
		try {
			const res = await fetch("/api/provider-nodes/validate", {
				...
				body: JSON.stringify({
					...
					chatPath:
						formData.chatPath ||
						(isCcCompatible ? CC_COMPATIBLE_DEFAULT_CHAT_PATH : ""),
					modelsPath: isCcCompatible ? "" : formData.modelsPath,
				}),
			});
			const data = await res.json();
		...
		>
			<div className="flex flex-col gap-4">
				...
				<Input
					...
					hint={
						isCcCompatible
							? "Used for aliases such as prefix/model-id"
							: t("prefixHint")
					}
				/>
				{!isAnthropic && (
