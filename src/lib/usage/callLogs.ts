...
function protectPipelinePayloads(payloads: unknown): RequestPipelinePayloads | null {
	...
	for (const [key, value] of Object.entries(payloads as JsonRecord)) {
		...
		protectedPayloads[key as keyof RequestPipelinePayloads] = protectPayloadForLog(
			value
		) as never;
	}
...
export function cleanupOrphanCallLogFiles(baseDir = getCallLogsDir()) {
	...
	try {
		...
		console.error(
			"[callLogs] Failed to prune orphan request artifacts:",
...
function mapSummaryRow(row: CallLogSummaryRow) {
	...
	return {
		...
		tokens: {
			...
			cacheWrite:
				row.tokens_cache_creation != null ? toNumber(row.tokens_cache_creation) : null,
	...
	if (artifactRelPath) {
		...
		if (artifactResult.state === "ready" && artifactResult.artifact) {
			return {
				...
				pipelinePayloads:
					artifactResult.artifact.pipeline ?? buildLegacyPipelinePayloads(id),
				hasPipelineDetails:
					Boolean(artifactResult.artifact.pipeline) || entry.hasPipelineDetails,
			};
		}
