	...
	if (stat.isDirectory()) {
		...
		for (const entry of entries) {
			addPathToZip(
				...
				path.posix.join(archivePath, entry)
			);
		}
		return;
	}
...
export function migrateUsageJsonToSqlite() {
	...
	if (USAGE_JSON_FILE && fs.existsSync(USAGE_JSON_FILE)) {
		try {
			...
			if (history.length > 0) {
				console.log(
					`[usageDb] Migrating ${history.length} usage entries from JSON → SQLite...`
				);
	...
	if (CALL_LOGS_JSON_FILE && fs.existsSync(CALL_LOGS_JSON_FILE)) {
		try {
			...
			if (logs.length > 0) {
				console.log(
					`[usageDb] Migrating ${logs.length} call log entries from JSON → SQLite...`
				);
        ...
        `);
				...
				const tx = db.transaction(() => {
					for (const log of logs) {
						const id =
							log.id || `${Date.now()}-${Math.random().toString(36).slice(2, 6)}`;
						...
						insert.run({
							...
							requestSummary: buildLegacyRequestSummary(
								log.requestType,
