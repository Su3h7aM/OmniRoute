...
export class GuardrailRegistry {
	...
	async runPreCallHooks<TPayload = unknown>(payload: TPayload, context: GuardrailContext = {}) {
		...
		for (const guardrail of this.guardrails) {
			...
			try {
				...
				logger.warn?.("GUARDRAIL", `${guardrail.name} pre-call failed open`, {
					error: message,
				});
			}
		}
	...
	async runPostCallHooks<TResponse = unknown>(
		response: TResponse,
		...
		for (const guardrail of this.guardrails) {
			...
			try {
				...
				logger.warn?.("GUARDRAIL", `${guardrail.name} post-call failed open`, {
					error: message,
				});
			}
		}
