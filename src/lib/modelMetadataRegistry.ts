...
function extractCandidateMatches(modelId: string) {
	...
	for (const [providerAlias, models] of Object.entries(PROVIDER_MODELS)) {
		for (const entry of models || []) {
			...
			const providerId =
				Object.entries(PROVIDER_ID_TO_ALIAS).find(
...
export function getCanonicalModelMetadata(input: {
	...
	return {
		...
		providerLabel:
			(provider &&
				(AI_PROVIDERS as Record<string, JsonRecord>)[provider]?.name?.toString()) ||
			null,
...
function buildAliasCandidates(alias: string) {
	...
	const candidates = new Map<
		string,
		{ provider: string; providerAlias: string; model: string }
	>();
		...
		return {
			...
			error: {
				...
				candidates: candidates.map(
					(candidate) => `${candidate.providerAlias}/${candidate.model}`
				),
			},
		};
	}
