...
async function validateDatabricksProvider({ apiKey, providerSpecificData = {} }: any) {
	...
	return validateDirectChatProvider({
		...
		body: {
			model:
				providerSpecificData.validationModelId || "databricks-meta-llama-3-3-70b-instruct",
			messages: [{ role: "user", content: "test" }],
			max_tokens: 1,
		},
		providerSpecificData,
...
export async function validateClaudeCodeCompatibleProvider({
	...
	const modelsPath =
		providerSpecificData?.modelsPath || CLAUDE_CODE_COMPATIBLE_DEFAULT_MODELS_PATH;
...
async function validatePerplexityWebProvider({ apiKey, providerSpecificData = {} }: any) {
	try {
		...
		if (response.status === 401 || response.status === 403) {
			return {
				valid: false,
				error: "Invalid Perplexity session cookie — re-paste __Secure-next-auth.session-token from perplexity.ai",
			};
		}
