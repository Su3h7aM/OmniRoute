...
export function normalizeIntelligentRoutingConfig(config: unknown): IntelligentRoutingConfig {
	...
	return {
		candidatePool: Array.isArray(configRecord.candidatePool)
			? configRecord.candidatePool.filter(
					(value): value is string => typeof value === "string"
				)
			: [],
		explorationRate: Math.min(
			1,
			Math.max(0, toFiniteNumber(configRecord.explorationRate) ?? 0.05)
		),
		modePack:
		...
		weights: {
			...
			latencyInv:
				toFiniteNumber(rawWeights.latencyInv) ?? DEFAULT_INTELLIGENT_WEIGHTS.latencyInv,
			taskFit: toFiniteNumber(rawWeights.taskFit) ?? DEFAULT_INTELLIGENT_WEIGHTS.taskFit,
			stability:
				toFiniteNumber(rawWeights.stability) ?? DEFAULT_INTELLIGENT_WEIGHTS.stability,
			tierPriority:
...
export function extractIntelligentHealthState(health: unknown): {
	...
	const providerBreakers = Object.entries(providerHealth).map(([provider, status]) => {
		...
		return {
			...
			lastFailure:
				typeof statusRecord.lastFailure === "string" ? statusRecord.lastFailure : null,
		};
	});
		...
		? healthRecord.circuitBreakers
				.map((entry) => {
					...
					return {
						...
						lastFailure:
							typeof breaker.lastFailure === "string" ? breaker.lastFailure : null,
					};
				})
