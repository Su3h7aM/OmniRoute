...
export async function computeAnalytics(
	...
	await Promise.all(
		[...uniquePairs].map(async (key) => {
			...
			let pricing = (await getPricingForModel(provider, model)) as Record<
				string,
				unknown
			> | null;
			if (!pricing) {
	...
	for (let i = 0; i < entries.length; i++) {
		...
		dailyByModelMap[dateKey][modelShort] =
			(dailyByModelMap[dateKey][modelShort] || 0) + totalTkns;
		...
		.map((m) => ({
			...
			pct:
				summary.totalTokens > 0
					? ((m.totalTokens / summary.totalTokens) * 100).toFixed(1)
					: "0",
		}));
