...
const goldenSet = {
	...
	cases: [
		...
		},
		{
			...
			model: "gpt-4o",
			input: {
				messages: [{ role: "user", content: "List 3 colors as a markdown bullet list" }],
			},
	...
	cases: [
		{
			...
			input: {
				messages: [
					{
						role: "user",
						content: "Write a FizzBuzz function in Python for numbers 1 to 15",
					},
				],
			},
			expected: { strategy: "contains", value: "def " },
				...
				messages: [
					{
						role: "user",
						content:
							"Find the bug: function sum(a, b) { return a * b; }. What should the fix be?",
					},
				],
			},
	...
	cases: [
		{
			...
			input: {
				messages: [
					{
						role: "user",
						content: "Translate to Portuguese: 'The weather is beautiful today'",
					},
				],
			},
				...
				messages: [
					{
						role: "user",
						content:
							"What language is this sentence in? 'Guten Morgen, wie geht es Ihnen?'",
					},
				],
			},
		...
		},
		{
			...
			input: {
				messages: [
					{
						role: "user",
						content: "How do you say 'thank you' in Japanese? Include romaji.",
					},
				],
			},
