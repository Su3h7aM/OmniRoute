...
function deepMergeCompatByProtocol(
	...
	for (const key of Object.keys(patch) as ModelCompatProtocolKey[]) {
		...
		const hasDelta =
			Object.hasOwn(deltas, "normalizeToolCallId") ||
			Object.hasOwn(deltas, "preserveOpenAIDeveloperRole") ||
			Object.hasOwn(deltas, "upstreamHeaders");
...
export function mergeModelCompatOverride(
	...
	const hasPreserveFlag = Object.hasOwn(next, "preserveOpenAIDeveloperRole");
	...
	const hasHiddenFlag = Object.hasOwn(next, "isHidden");
	if (
	...
	const merged = models.map((m) => {
		...
		return {
			...
			supportedEndpoints: m.supportedEndpoints ||
				(prev as any)?.supportedEndpoints || ["chat"],
...
export async function updateCustomModel(
	...
	const currentCompat = (current as JsonRecord).compatByProtocol as
		| CompatByProtocolMap
		| undefined;
	...
	if (Object.hasOwn(updates, "preserveOpenAIDeveloperRole")) {
	...
	if (Object.hasOwn(updates, "upstreamHeaders")) {
...
export function getModelNormalizeToolCallId(
	...
	if (protocol && co?.compatByProtocol?.[protocol]) {
		...
		if (Object.hasOwn(pc, "normalizeToolCallId")) {
			return Boolean(pc.normalizeToolCallId);
		}
	}
...
export function getModelPreserveOpenAIDeveloperRole(
	...
	if (m) {
		...
		if (Object.hasOwn(m, "preserveOpenAIDeveloperRole")) {
	...
	if (protocol && co?.compatByProtocol?.[protocol]) {
		const pc = co.compatByProtocol[protocol]!;
		if (Object.hasOwn(pc, "preserveOpenAIDeveloperRole")) {
			return Boolean(pc.preserveOpenAIDeveloperRole);
		}
	}
	if (co && Object.hasOwn(co, "preserveOpenAIDeveloperRole")) {
...
export function getModelUpstreamExtraHeaders(
	...
	if (m) {
		...
		if (protocol) {
			...
			if (fromProto && typeof fromProto === "object") {
				Object.assign(
					base,
					sanitizeUpstreamHeadersMap(fromProto as Record<string, unknown>)
				);
			}
		}
		return base;
	...
	if (co?.upstreamHeaders) {
		Object.assign(
			base,
			sanitizeUpstreamHeadersMap(co.upstreamHeaders as Record<string, unknown>)
		);
	}
