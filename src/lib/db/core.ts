		...
		readRows(db) {
			return (
				...
				(row) =>
					...
					!SKIP_PRESERVE_NAMESPACES.has(row.namespace)
			);
		},
	},
	...
	const restore = db.transaction(() => {
		for (const table of snapshot.preservedTables) {
			...
			if (restoreColumns.length === 0) {
				throw new Error(
					`No compatible columns remain for preserved table "${table.table}"`
				);
			}
...
function offloadLegacyCallLogDetails(db: SqliteDatabase) {
  ...
  `);
	...
	try {
		...
		db.exec("VACUUM");
		console.log(
			`[DB] Offloaded ${pendingRows.length} legacy call log detail row(s) to artifacts.`
		);
	} catch (error: unknown) {
		const message = error instanceof Error ? error.message : String(error);
