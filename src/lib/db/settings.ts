	...
	if (model) {
		...
		if (row) {
			...
			if (Object.keys(models).length === 0) {
				db.prepare("DELETE FROM key_value WHERE namespace = 'pricing' AND key = ?").run(
					provider
				);
			} else {
				db.prepare(
...
export async function setLKGP(comboName: string, modelId: string, providerId: string) {
	...
	).run(key, JSON.stringify(providerId));
}
		...
		if (config.combos && Object.keys(config.combos).length > 0) {
			...
			for (const comboRow of combos) {
				...
				if (comboId && config.combos[comboId]) {
					try {
						const comboRaw =
							typeof comboRecord.data === "string" ? comboRecord.data : null;
...
export async function getCacheMetrics() {
    ...
    `
		...
		const estimatedCostSaved =
			Math.round(
				(tokensSaved / 1_000_000) * AVG_INPUT_PRICE_PER_MILLION * CACHE_DISCOUNT * 100
			) / 100;
