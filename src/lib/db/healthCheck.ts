...
function buildDisabledCombo(row: ComboRow, checkedAt: string): JsonRecord {
	...
	return {
		...
		repairNote: buildRepairNote(
			"Combo payload was rebuilt after invalid JSON was detected.",
			now
		),
	};
}
...
function repairOrphanDomainRows(
	...
	if (!hasRows(db, table)) return 0;
	return db
		.prepare(`DELETE FROM ${table} WHERE api_key_id NOT IN (SELECT id FROM api_keys)`)
		.run().changes;
}
...
function countInvalidJsonRows(
	...
	if (!hasRows(db, table)) return 0;
	const rows = db.prepare(`SELECT ${column} FROM ${table}`).all() as Array<
...
export function runDbHealthCheck(
	...
	const ensureBackupBeforeRepair = () => {
		if (
			!autoRepair ||
			backupAttempted ||
			typeof options.createBackupBeforeRepair !== "function"
		) {
			return;
		}
