					...
					<thead className="text-xs text-text-muted uppercase bg-black/[0.02] dark:bg-white/[0.02]">
						<tr>
							...
							>
								Output{" "}
								<SortIndicator
									active={sortBy === "completionTokens"}
									sortOrder={sortOrder}
								/>
							</th>
							<th
							...
							>
								Cost{" "}
								<SortIndicator active={sortBy === "cost"} sortOrder={sortOrder} />
							</th>
						</tr>
					</thead>
							...
							>
								<td className="px-4 py-2.5">
									<span
										...
										title={row.apiKeyName || row.apiKeyId || "unknown"}
									>
						...
						<div key={i} className="flex items-center justify-between gap-2 text-xs">
							<div className="flex items-center gap-1.5 min-w-0">
								<span
									...
									style={{ backgroundColor: seg.fill }}
								/>
								<span className="truncate text-text-main capitalize">
									{seg.name}
								</span>
							</div>
				...
				<table className="w-full text-sm">
					...
					<tbody className="divide-y divide-border">
						{sorted.map((p, i) => {
							const pct =
								totalTokens > 0
									? ((p.totalTokens / totalTokens) * 100).toFixed(1)
									: "0";
							return (
								<tr
									key={p.provider}
