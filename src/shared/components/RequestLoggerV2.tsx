...
export default function RequestLoggerV2() {
	...
	const uniqueModels = [
		...new Set(
			logs.flatMap((l) => [l.model, l.requestedModel]).filter((value) => Boolean(value))
		),
	].sort();
						...
						<table className="w-full text-left border-collapse text-xs">
							...
							<tbody className="divide-y divide-border/30">
								{sortedLogs.map((log) => {
									...
									const compatLabel = getProviderDisplayLabel(
										log.provider,
										providerNodes
									);
												...
												<td className="px-3 py-2 text-right whitespace-nowrap font-mono">
													{(() => {
														...
														return (
															<span
																className={color}
																title={`${tps.toFixed(2)} tokens/sec`}
															>
																{formatTps(tps)}
															</span>
														);
