		...
		const loadProxy = async () => {
			try {
				...
				const assignmentRes = await fetch(
					`/api/settings/proxies/assignments?${assignmentParams}`
				);
				if (assignmentRes.ok) {
					const assignmentPayload = await assignmentRes.json();
					const items = Array.isArray(assignmentPayload?.items)
				...
				if (res.ok) {
					...
					if (proxy?.host) {
						...
						setProxyType(
							hasTypeOption ? normalizedType : PROXY_TYPES[0]?.value || "http"
						);
						setHost(proxy.host || "");
						setPort(proxy.port || "");
		...
		try {
			...
			if (mode === "saved") {
				...
				if (res.ok) {
					...
					await fetch(`/api/settings/proxy?${clearParams.toString()}`, {
						method: "DELETE",
					});
				}
			} else {
				...
				if (!clearAssignmentRes.ok) {
					setFormError(
						clearAssignmentPayload?.error?.message || t("errorClearSavedProxy")
					);
					return;
				}
