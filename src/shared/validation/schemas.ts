...
function validateProviderSpecificData(
	...
	if (routingTags !== undefined && routingTags !== null) {
		if (!Array.isArray(routingTags) || routingTags.length > 50) {
			...
			routingTags.some(
				(tag) =>
					typeof tag !== "string" || tag.trim().length === 0 || tag.trim().length > 64
			)
		) {
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
	...
	if (excludedModels !== undefined && excludedModels !== null) {
		if (typeof excludedModels === "string") {
			...
			ctx.addIssue({
				code: z.ZodIssueCode.custom,
				message:
					"providerSpecificData.excludedModels must be an array with at most 100 items",
				path: ["excludedModels"],
			});
		} else if (
			excludedModels.some(
...
export const setBudgetSchema = z
	.object({
		...
		dailyLimitUsd: z.coerce
			.number()
			.positive("dailyLimitUsd must be greater than zero")
			.optional(),
...
const upstreamHeaderNameSchema = z
	...
	.max(128)
	.refine((s) => !/[\r\n\0]/.test(s), {
		message: "header name cannot contain control characters",
	})
	...
	.object({
		...
		params: z
			...
			.refine(
				...
				"params must contain at least one path"
			),
	})
	.strict();
