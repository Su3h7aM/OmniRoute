...
test("skills pipeline can be disabled via skillsEnabled flag without crashing", async () => {
	...
	assert.equal(
		hasSkillTools,
		false,
		"skill tools should NOT be injected when skillsEnabled=false"
	);
		...
		buildRequest({
			...
			body: {
				...
				messages: [
					{
						role: "user",
						content: "Search the web for the latest OmniRoute release notes",
					},
				],
				...
				input: [
					{
						...
						content: [
							{
								type: "input_text",
								text: "Search the web for the latest OmniRoute roadmap",
							},
						],
					},
				],
