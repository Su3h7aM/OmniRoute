...
const { resetAllAvailability, setModelUnavailable } = await import(
	"../../src/domain/modelAvailability.ts"
);
	...
	const response = await handleChat(
		buildRequest({
			...
			body: {
				...
				messages: [
					{ role: "user", content: [{ type: "text", text: "Hello from Claude client" }] },
				],
			},
		})
	);
...
test("chat pipeline surfaces upstream 500 responses as structured errors", {
	timeout: 10000,
}, async () => {
