...
test("memory injection respects the configured token budget", async () => {
	...
	assert.doesNotMatch(
		fetchCalls[0].messages[0].content,
		/Older preference that should be trimmed/
	);
});
...
test("extraction→storage: extractFactsFromText output persists via createMemory", async () => {
	...
	const text =
		"I prefer TypeScript. I usually write tests first. I'll use Vitest for unit tests.";
...
test("retrieval→injection: retrieveMemories feeds into injectMemory context", async () => {
	...
	assert.equal(
		injected.messages[0].role,
		"system",
		"memory should be injected as system message"
	);
