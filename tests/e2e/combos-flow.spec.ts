...
test.describe("Combos flow", () => {
	test("applies template, creates combo, and runs quick test CTA", async ({ page }) => {
			...
			if (method === "POST") {
				...
				const payload =
					payloadRaw && typeof payloadRaw === "object"
						? (payloadRaw as ComboCreatePayload)
						: {};
		...
		await expect(
			comboDialog.locator('[data-testid="combo-builder-stage-steps"]')
		).toBeVisible();
		await expect(comboNextButton).toBeDisabled();
		await comboDialog.locator('[data-testid="combo-builder-provider"]').selectOption("openai");
		await comboDialog
			.locator('[data-testid="combo-builder-model"]')
			.selectOption("qa-test-model");
		await comboDialog
			.locator('[data-testid="combo-builder-account"]')
			.selectOption("conn-openai");
