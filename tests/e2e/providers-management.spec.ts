		...
		window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
			...
			if (detailMatch) {
				...
				if (method === "PUT") {
						...
						connection.id === connectionId
							? {
									...
									providerSpecificData: {
										...
										...(payload.providerSpecificData as
					...
					const updated = state.connections.find(
...
test.describe("Providers management", () => {
	...
	test("adds, edits, retests, deletes, and validates provider connections through the UI", async ({
		...
		await expect
			.poll(async () => (await readProviderMockState(page)).connections.length)
			.toBe(1);
