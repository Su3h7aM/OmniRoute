...
test.serial(
	"getCloudflaredTunnelStatus resolves a PATH-installed binary when no managed install exists",
	async () => {
...
test.serial(
	"startCloudflaredTunnel reaches running state and stopCloudflaredTunnel clears persisted runtime state",
	async () => {
		...
		childProcess.spawn = (command, args, options) => {
			...
			setTimeout(() => {
				child.stdout.write(
					Buffer.from(
						"INF Visit https://violet-cloud.trycloudflare.com to inspect the tunnel\n"
					)
				);
			}, 25);
...
test.serial(
	"startCloudflaredTunnel records an error state when the child exits before a public tunnel URL is available",
	async () => {
