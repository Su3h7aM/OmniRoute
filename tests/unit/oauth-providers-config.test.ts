...
test("every registered OAuth provider has a valid config object, flow type and token mapper", () => {
	...
	for (const [providerId, provider] of Object.entries(PROVIDERS)) {
		...
		assert.ok(
			allowedFlowTypes.has(provider.flowType),
			`${providerId} has unsupported flowType`
		);
...
test("Cline decodes embedded callback payloads without using the network", async () => {
	...
	const tokens = await PROVIDERS.cline.exchangeToken(
		CLINE_CONFIG,
		encodedCode,
		"http://localhost"
	);
...
test("Qwen and Kimi Coding execute mocked device-code flows and token mapping", async () => {
	...
	const qwenPoll = await PROVIDERS.qwen.pollToken(
		QWEN_CONFIG,
		qwenDevice.device_code,
		"verifier"
	);
...
test("Kiro and KiloCode execute mocked device-code flows across their custom endpoints", async () => {
	...
	const kiloApproved = await PROVIDERS.kilocode.pollToken(
		KILOCODE_CONFIG,
		kiloDevice.device_code
	);
