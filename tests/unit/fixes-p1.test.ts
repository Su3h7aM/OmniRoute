...
test("restoreDbBackup clears stale sqlite sidecars before reopen", {
	skip: isWindows,
}, async () => {
	await resetStorage();
	...
	for (const suffix of ["-wal", "-shm", "-journal"]) {
		...
		assert.equal(
			text.includes("STALE-"),
			false,
			`sidecar ${suffix} still contains stale marker`
		);
	}
...
test("proxy test route validates JSON, schema, and proxy types before dispatching", async () => {
	...
	assert.match(
		socks4Body.error.message,
		/proxy\.type must be http, https, or socks5|proxy\.type must be http or https/i
	);
