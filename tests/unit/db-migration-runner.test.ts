...
function buildMockMigrationFiles(startVersion, endVersion, prefix) {
	...
	for (let version = startVersion; version <= endVersion; version++) {
		...
		const fileName =
			version === 1 ? "001_initial_schema.sql" : `${padded}_${prefix}_${padded}.sql`;
	...
	try {
		...
		assert.equal(
			(
				db
					.prepare(
						"SELECT COUNT(*) AS count FROM _omniroute_migrations WHERE version = ?"
					)
					.get("001") as any
			).count,
			1
			...
			const count = withMockedMigrationFs(
				{
					...
					"025_call_logs_summary_storage.sql":
						"CREATE TABLE call_log_summary_shadow (id INTEGER);",
				},
				() => runner.runMigrations(db)
			);
