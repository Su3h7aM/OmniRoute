...
test.serial(
	"fetchModelsDev caches successful responses and rejects invalid JSON or non-ok responses",
	async () => {
...
test.serial(
	"modelsDev capability helpers coerce false/null values and ignore malformed rows",
	async () => {
		...
		if (String(sql).includes("SELECT key, value FROM key_value")) {
			return {
				all: () => [
					...
					{
						key: "openai",
						value: JSON.stringify({ "gpt-4o": { input: 2.5, output: 10 } }),
					},
				],
			};
		}
...
test.serial(
	"syncModelsDev supports dry-run mode, persistence, capability toggles, and failure reporting",
	async () => {
