...
test("handleComboChat records per-target metrics separately when the same model repeats with different accounts", async () => {
	...
	assert.deepEqual(calls, [
		"openai/gpt-4o-mini:conn-openai-a",
		"openai/gpt-4o-mini:conn-openai-b",
	]);
...
test("handleComboChat falls through invalid JSON and embedded 200 error bodies before succeeding", async () => {
	...
	const result = await handleComboChat({
		...
		handleSingleModel: async (_body, modelStr) => {
			...
			if (modelStr === "model-b") {
				return new Response(
					JSON.stringify({ error: { message: "embedded upstream failure" } }),
					{
						status: 200,
						headers: { "content-type": "application/json" },
					}
	...
	const result = await handleComboChat({
		...
		handleSingleModel: async (_body, modelStr) => {
			...
			if (modelStr === "model-a") {
				return new Response(
					JSON.stringify({
						error: { message: "unsupported message role for this provider" },
					}),
					{
						status: 400,
						headers: { "content-type": "application/json" },
