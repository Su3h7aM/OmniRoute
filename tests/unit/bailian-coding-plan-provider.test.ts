...
const { APIKEY_PROVIDERS, OAUTH_PROVIDERS } = await import(
...
const { validateBody, createProviderSchema, updateProviderConnectionSchema } = await import(
...
const { getStaticModelsForProvider } = await import(
...
test("getStaticModelsForProvider returns undefined for non-static providers", () => {
	...
	for (const provider of nonStaticProviders) {
		...
		assert.equal(
			models,
			undefined,
			`Should return undefined for non-static provider: ${provider}`
		);
	}
});
...
test("POST /api/providers validation: bailian-coding-plan with baseUrl passes schema", () => {
	...
	assert.equal(
		validation.success,
		true,
		"Schema should accept valid bailian-coding-plan payload"
	);
	if (validation.success) {
