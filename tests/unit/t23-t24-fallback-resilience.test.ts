...
const { handleComboChat, shouldFallbackComboBadRequest } = await import(
...
test("T24: combo awaits short 503 cooldown before falling through to next model", async () => {
	...
	const waitLog = log.entries.find(
		(e) => e.msg.includes("Waiting") && e.msg.includes("fallback")
	);
	assert.ok(waitLog);
});
...
test("T24: all inactive accounts return 503 service_unavailable (not 406)", async () => {
	const result = await handleComboChat({
		...
		handleSingleModel: async () => {
			throw new Error(
				"handleSingleModel should not be called when all models are unavailable"
			);
		},
...
test("combo bad-request fallback helper keeps generic 400s terminal", () => {
	...
	assert.equal(
		shouldFallbackComboBadRequest(
			400,
			"One or more of the provided message roles is not valid"
		),
		true
	);
