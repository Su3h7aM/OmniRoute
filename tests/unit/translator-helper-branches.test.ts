...
test("claudeHelper validates content, ordering and request preparation branches", () => {
	...
	assert.equal(
		claudeHelper.hasValidContent({ content: [{ type: "tool_use", id: "call" }] }),
		true
	);
			...
			messages: [
				{
					role: "assistant",
					content: [
						{ type: "text", text: "keep cache", cache_control: { type: "ephemeral" } },
					],
				},
			],
...
test("toolCallHelper normalizes ids, links tool responses and inserts missing tool results", () => {
	...
	assert.deepEqual(toolCallHelper.fixMissingToolResponses({ messages: null }), {
		messages: null,
	});
});
