				...
				candidates: [
					{
						content: {
							parts: [
								{ text: "revised prompt" },
								{ inlineData: { data: "YmFzZTY0LWdlbWluaQ==" } },
							],
						},
					},
				],
...
test("handleImageGeneration polls NanoBanana task results and converts URLs to base64 when requested", async () => {
	...
	globalThis.fetch = async (url, _options = {}) => {
		...
		if (
			stringUrl === "https://api.nanobananaapi.ai/api/v1/nanobanana/record-info?taskId=task-1"
		) {
			return new Response(
				JSON.stringify({
					data: {
				...
				JSON.stringify({
					"image-1": {
						outputs: {
							9: {
								images: [
									{ filename: "frame.png", subfolder: "out", type: "output" },
								],
							},
						},
					},
...
test("handleImageGeneration logs Nebius fallback attempts before succeeding", async () => {
	...
	try {
		...
		assert.equal(
			log.entries.some(
				(entry) => entry.level === "info" && /trying fallback/.test(entry.message)
			),
			true
		);
	} finally {
...
test("handleImageGeneration handles NanoBanana missing statusUrl, failed tasks and empty completed payloads", async () => {
	...
	try {
		...
		assert.equal(
			log.entries.some(
				(entry) =>
					entry.level === "warn" && /completed without image payload/.test(entry.message)
			),
			true
		);
	} finally {
