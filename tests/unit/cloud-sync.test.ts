...
async function loadCloudSync(label) {
	return import(
		`${cloudSyncModuleUrl}?cloudUrl=${encodeURIComponent(process.env.CLOUD_URL || process.env.NEXT_PUBLIC_CLOUD_URL || "")}&case=${label}-${Date.now()}-${Math.random()}`
	);
}
...
test.serial(
	"cloudSync returns a generic error when the API responds with a non-OK status",
	async () => {
		setCloudUrl("https://cloud.example");
...
test.serial(
	"cloudSync syncs data upstream and refreshes only locally stale provider tokens",
	async () => {
		setCloudUrl("https://cloud.example");
