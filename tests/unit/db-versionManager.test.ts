...
describe("db/versionManager (logic)", () => {
	describe("upsertVersionManagerTool", () => {
		...
		test("should store config overrides as JSON", () => {
			const tool = toTool(
				upsertTool(testDb, { tool: "cfg", configOverrides: { port: 9999 } })
			);
	...
	describe("updateVersionManagerTool (partial)", () => {
		...
		test("should set fields to null", () => {
			...
			testDb
				.prepare(
					"UPDATE version_manager SET installed_version = NULL, pid = NULL WHERE tool = ?"
				)
				.run("nulls");
			const tool = toTool(
	...
	describe("updateToolHealth", () => {
		test("should update health_status and last_health_check", () => {
			...
			const tool = toTool(
				testDb.prepare("SELECT * FROM version_manager WHERE tool = ?").get("h")
			);
	...
	describe("updateToolVersion", () => {
		test("should update current_version", () => {
			...
			testDb
				...
				.run("7.0.0", "v1");
			assert.equal(
				testDb.prepare("SELECT * FROM version_manager WHERE tool = ?").get("v1")
					.current_version,
				"7.0.0"
			);
		});
		...
		test("should update installed_version", () => {
			...
			testDb
				...
				.run("7.0.0", "v2");
			assert.equal(
				testDb.prepare("SELECT * FROM version_manager WHERE tool = ?").get("v2")
					.installed_version,
				"7.0.0"
			);
		});
	...
	describe("setToolStatus", () => {
		test("should update status with pid", () => {
			...
			testDb
				.prepare(
					"UPDATE version_manager SET status = ?, pid = ?, error_message = ? WHERE tool = ?"
				)
...
describe("db/versionManager (module coverage)", () => {
	...
	test("updates health/version/status fields and deletes tools", async () => {
		...
		assert.equal(
			await versionManagerDb.setToolStatus("managed-tool", "running", 4321, "ok"),
			true
		);
