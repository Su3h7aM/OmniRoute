	...
	const claudeBody = {
		...
		messages: [
			...
			},
			{
				role: "assistant",
				content: [
					{ type: "text", text: "a1", cache_control: { type: "ephemeral", ttl: "10m" } },
				],
			},
...
test("chatCore refreshes GitHub credentials after 401 and retries with the refreshed Copilot token", async () => {
	...
	assert.equal(
		refreshedCredentials?.providerSpecificData?.copilotToken,
		"copilot-refreshed-token"
	);
	...
	const { calls, result } = await invokeChatCore({
		...
		responseFactory(_captured, seenCalls) {
			if (seenCalls.length === 1) {
				return new Response(
					JSON.stringify({
						error: { message: "You exceeded your current quota for Qwen." },
					}),
					{
						status: 429,
						headers: { "Content-Type": "application/json" },
...
test("chatCore persists Codex quota headers and scope cooldown on 429 responses", {
	timeout: 10000,
}, async () => {
...
test("chatCore falls back to a larger-context sibling when the request overflows context", async () => {
	...
	const { calls, result } = await invokeChatCore({
		...
		responseFactory(_captured, seenCalls) {
			if (seenCalls.length === 1) {
				return new Response(
					JSON.stringify({ error: { message: "maximum context exceeded" } }),
					{
						status: 400,
						headers: { "Content-Type": "application/json" },
					}
